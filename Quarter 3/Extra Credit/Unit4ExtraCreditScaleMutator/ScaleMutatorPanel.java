...
public class ScaleMutatorPanel extends JPanel
{
   ...
   private static final int SIZE=40;	            //default size of units being drawn
   ...
   private static int DELAY=0;	                  //#miliseconds delay between each time the screen refreshes for the timer
   ...
   {
      playChart = false;
      chartIndex = -1;                          //start at -1 so there is 1 note delay between pressing play and the first note
      ...
      {  
         ...
         if(k==KeyEvent.VK_P || k==KeyEvent.VK_SPACE)   //play the scale
         {
            //playNow(scale);
            playChart = !playChart;
            repaint();			
         ...
         {   
            if(opt==1)                    //sort
               ScaleMutator.sort(scale);
            else if(opt==2)               //reverse
               scale = ScaleMutator.createReversal(scale);
            else if(opt==3)               //palindrome
               scale = ScaleMutator.createAPalindrome(scale); 
            else if(opt==4)               //shuffle with self
               scale = ScaleMutator.createSelfAlternation(scale);
            else if(opt==5)               //shuffle with reverse
               scale = ScaleMutator.createAMerge(scale, ScaleMutator.createReversal(scale));
            else if(opt==6)               //scramble
               scale = ScaleMutator.createScrambledVersion(scale);
            else if(opt==7)               //mix with root
               scale = ScaleMutator.createAlternationWithNote(scale, scale[0]); 
            else if(opt==8)               //triads
               scale = ScaleMutator.createTriadVersion(scale);
            else if(opt==9)               //restore original
               scale = scaleOrig.clone();
            else if(opt==0)               //extra credit mutation
               scale = ScaleMutator.feedOberlesEgo(scale, 3);
         }
      }
   ...
   public void paintComponent(Graphics g)
   {
      ...
      else
      {
         ...
         g.drawString("(P)lay    (Q)uit        (S)tart over               (W)rite to MIDI", x, y+=(SIZE));
         g.drawString("(0) Impress Dr. Oberle  (1) Sort     (2) Reverse   (3) Palindrome", x, y+=(SIZE));
         g.drawString("(4) Alternate Self    (5) Alternate w/ Reverse   (6) Scramble", x, y+=(SIZE));
         g.drawString("(7) Alt With First Note   (8) Make Triads     (9) Restore", x, y+=(SIZE));
         g.drawString("" + scaleString(scale, false), x, y+=(SIZE));
         y+=(SIZE*6);
         int [] leveled = levelScale(scale);
         int radius = SIZE;                  //decrease radius of drawn notes on the screen to make them fit
         while(leveled.length*radius > SCREEN_WIDTH && radius > 1)
            radius--;
         for(int i=0; i<leveled.length; i++) //draw the notes as scaled ovals on the screen
         {
            int note = leveled[i];
            int red = ((note+1)*15)%256;     //change color of the note depending on its position
            int green = ((scaleIndex+1) * 63)%256;
            int blue = (256-((note+1)*15))%256;
            if(playChart && i==chartIndex)
               g.setColor(Color.yellow);
            else
            ...
            if(radius >=10)
            {
               ...
               if(playChart && i==chartIndex)
                  ...
                  g.setColor(Color.black);      //draw black border around the note
               g.drawOval(x, y-(note*19), radius, SIZE);
            }
            else                             //switch to drawing rectangles if the raidus is too small
            {
               g.fillRect(x, y-(note*19), radius, SIZE);
               if(radius >= 3)               //don't draw black border around the note if the radius is too small       
               {
                  if(playChart && i==chartIndex)
                     g.setColor(Color.white);
                  else
                     g.setColor(Color.black);
                  g.drawRect(x, y-(note*19), radius, SIZE);
               }
            }
            x+=(radius);
         }
   ...
   private class Listener implements ActionListener
   {
      public void actionPerformed(ActionEvent e)	//this is called for each timer iteration
      {
         ...
         if(frame == Integer.MAX_VALUE)
            frame = 1;
         if(scale!=null && chartIndex >= scale.length)
         {
            playChart = false;
            chartIndex = -1;
         }           
         if(playChart)
         {
            if(frame % WHOLENOTE == 0 && chartIndex >= 0 && chartIndex < scale.length)
            {
               channels[0].allNotesOff(); 
               channels[0].noteOn(scale[chartIndex], VOLUME);
            } 
            else if(frame % WHOLENOTE == WHOLENOTE - 1)
            {
               chartIndex++;
            } 
         }
